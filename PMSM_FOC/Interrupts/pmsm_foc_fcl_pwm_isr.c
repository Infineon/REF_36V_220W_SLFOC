...
volatile int32_t TimerValue; // Variable for testing the max cpu utilization
...
#pragma diag_suppress=Ta023
  ...
  ProbeScope_Sampling();
  #endif
  PMSM_FOC_MiscWorks();           /* 480 ticks */
  /* System monitoring to detect configured errors */
  PMSM_FOC_SysMonitoring();       /* 306 ticks */
  //	XMC_GPIO_SetOutputLow(TEST_PIN2);

  /* For testing the max cpu utilization of the fast control loop */
  temp = XMC_CCU8_SLICE_GetTimerValue(CCU8_SLICE_ADC_TR);
  if(TimerValue < temp)
  {
    TimerValue = temp;
  }
