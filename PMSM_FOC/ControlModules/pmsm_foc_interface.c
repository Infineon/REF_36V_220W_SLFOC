...
void PMSM_FOC_MiscWorks(void)
{
  ...
  if (PMSM_FOC_CTRL.msm_state == PMSM_FOC_MSM_CLOSED_LOOP)
  {
    ...
    {
      if(PMSM_FOC_POSITION_CTRL.position_ctrl_enabled == FALSE)
      {
        ...
        // PMSM_FOC_CTRL.set_val_pot = (uint32_t)((MotorParam.ELECTRICAL_SPEED_HIGH_LIMIT_TS) * ADC.adc_res_pot) >> 12;
      }
    ...
    else if (PMSM_FOC_CTRL.ctrl_scheme_fun_ptr == &PMSM_FOC_VqVoltageCtrl)
    {
      ...
      {
        // optimization/Use a fix value for the startup ramping target instead of the value from the pot
        PMSM_FOC_LinearRampGenerator(USER_VQ_TARGET_VALUE_V, USER_VQ_RAMPUP_STEP, USER_VQ_RAMPDOWN_STEP, USER_VQ_RAMP_SLEWRATE, &PMSM_FOC_INPUT.ref_vq);
      }
      else
      {
        /* POT ADC values 0 ~ 2^12 represent motor target speed of ELECTRICAL_SPEED_LOW_LIMIT_TS ~ ELECTRICAL_SPEED_HIGH_LIMIT_TS:*/
