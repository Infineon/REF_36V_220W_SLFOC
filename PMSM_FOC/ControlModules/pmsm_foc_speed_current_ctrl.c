...
{
  /* Read phase current from ADC (140 timer ticks)*/
  PMSM_FOC_VADC_GetPhasecurrent(SVPWM.previous_sector_num, SVPWM.current_sector_num, &ADC);
  /* Remove offset and target scaling (78 timer ticks)*/
  ...
  /* Clarke transform  - Execute on CPU (74 timer ticks)*/
  ...
  /* Park transform including wait to get result execution time is 2.14us (206 timer ticks)*/
  /* PI Controller #1 - Speed PI controller execution time is 2.04us (196 timer ticks)*/
  /* Parallel computation takes 3.31us (318 timer ticks)*/
  /**************************************************************************************************************/
...
#pragma diag_default=Ta023
  ...
  /* PLL Imag and wait to get result take 2.14us (206 timer counts)*/
  /* Torque PI Controller takes 1.98us (190 timer ticks)*/
  /* Parallel computation takes 3.92us (376 timer ticks)*/
  /**************************************************************************************************************/
...
#pragma diag_default=Ta023
  ...
  /* Vref calculation and wait to get result take 2.38us (228 timer ticks)*/
  /* PI controller takes 2.23us (214 timer ticks)*/
  /* Parallel computation takes 4.44us (426 timer ticks)*/
  /**************************************************************************************************************/
...
#pragma diag_default=Ta023
  ...
  /* Car2Pol calculation and wait to get result take 2.62us (252 timer ticks)*/
  /* CPU execution takes 2.00us (192 timer ticks)*/
  /* Paralle computation takes 3.75us (360 timer ticks)*/
  /**************************************************************************************************************/
  /* CORDIC - Car2Pol */
  ...
  /* CPU execution. 154 ticks */
  /**************************************************************************************************************/
  if(PMSM_FOC_CTRL.transition_status == PMSM_FOC_MOTOR_STATUS_STABLE)
  {
    PMSM_FOC_PI_Controller((PMSM_FOC_POSITION_CTRL.ref_position_int16), (PMSM_FOC_POSITION_CTRL.position_cnt_int16), &PMSM_FOC_POSITION_PI);
    PMSM_FOC_CTRL.set_val_pot = PMSM_FOC_POSITION_PI.uk;
  }
}
