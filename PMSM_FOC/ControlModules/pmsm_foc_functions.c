 ...
 ********************************************************************************************************************/
// For testing
volatile int32_t SWTest1, SWTest2, SWTest3, SWTest4, SWTest5;
...
void PMSM_FOC_PI_InitIks(void)
{
  ...
  /* Init position PI controller. */
  PMSM_FOC_POSITION_PI.ik = 0;
  PMSM_FOC_POSITION_PI.ik <<= PMSM_FOC_POSITION_PI.scale_kp_ki;
...
#pragma diag_default=Ta023
	...
	/**************************************************************************************************************/
  if(PMSM_FOC_POSITION_CTRL.position_ctrl_enabled)    /* 410 ticks */
  {
    // Do position control if it is enabled
    PMSM_FOC_PositionCtrl();
  }
  if(PMSM_FOC_STALL_DETECTION.Stall_detection_enabled)  /* 300 ticks */
  {
    // Do stall detection
    PMSM_FOC_Stall_Detection();
  }
  // On-line adjustment for PLL Kp
  PLL_Online_Adjust();                                  /* 244 ticks */
	/**************************************************************************************************************/
}
