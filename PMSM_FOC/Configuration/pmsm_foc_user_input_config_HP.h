...
#ifndef USER_INPUT_CONFIG_HP_H_
#define USER_INPUT_CONFIG_HP_H_
...
#define VOLTAGE_DIVIDER_R_LOW                       (4.7f)                         /*!< DC link voltage sense resistor divider low side value ***fixed for IMD700A 3S eval board**/
...
#define USER_ROTOR_PRE_ALIGN_METHOD                 (ROTOR_PRE_ALIGNMENT)                /*!< 1. ROTOR_PRE_ALIGNMENT 2. ROTOR_PRE_ALIGN_NONE */
#define USER_MOTOR_STARTUP_METHOD                   (MOTOR_STARTUP_DIRECT_FOC)      /*!< 1. MOTOR_STARTUP_DIRECT_FOC 2. MOTOR_STARTUP_VF_OPEN_LOOP */

#define USER_ROTOR_PRE_ALIGNMENT_V_RAMP_RATE        (18.0F)         /*!< in volt/sec. Voltage is ramped to USER_ROTOR_PRE_ALIGNMENT_VOLTAGE_V and then maintained for USER_ROTOR_PRE_ALIGNMENT_TIME_MS*/
#define USER_ROTOR_PRE_ALIGNMENT_VOLTAGE_V          (3.0F)           /*!< in volts  should be less than PMSM_FOC_SL_DC_LINK_VOLT */
#define USER_ROTOR_PRE_ALIGNMENT_TIME_MS            (500.0F)         /*!< Rotor startup alignment time in mSec. Min range: 1/PWM_Freuency */

#define USER_VQ_INITIAL_VALUE_V                     (3.7F)                          /*!<  Direct FOC Startup - Vq value initial value based upon load and max current */
/* optimization/Use this fix value for startup but not the value that comes from the POT */
#define USER_VQ_TARGET_VALUE_V                      (uint32_t) (32767.0F * (USER_VQ_INITIAL_VALUE_V * 1.2F) / (USER_VDC_LINK_V / USER_SQRT_3_CONSTANT)) /* Vq target value during startup */
...
#define USER_REF_SETTING                            (DISABLED)                       /*!< POTENTIOMETER 1. DISABLED 2. ENABLED */
#define USER_TH_POT_ADC                             (10U)                           /*!< Threshold POT ADC that Motor can enter or exit Motor idle state. ***fixed 50U**/
 ...
 * Motor group
 ***************************/
#ifdef   MOTOR0_PMSM_FOC_MOTOR
#define  USER_MOTOR_R_PER_PHASE_OHM                 (0.202f)       /*!< Motor Resistance per phase in Ohm for PMSM Motor */
#define  USER_MOTOR_LS_PER_PHASE_uH                 (473.0f)      /*!< Motor Inductance Ls per phase in uH  */
#define  USER_MOTOR_POLE_PAIR                       (15.0f)        /*!< Motor Pole Pairs */
#define  USER_SPEED_HIGH_LIMIT_RPM                  (380U)       /*!< No load Speed of User Motor*/
#define  USER_SPEED_LOW_LIMIT_RPM                   (10U)        /*!< Min Speed of User Motor, original 200U*/
#define  NO_LOAD_SPEED_RPM                          (450.0f)     /* No-load speed for calculation of the BEMF constant */
#define  STALL_TORQUE_NM 	                          (10.0f)       /* Stall torque threshold */
#define  GEAR_RATIO                                 (1.0f)      /* Define the ratio if there is a gearbox. Otherwise define as 1.0f */
#endif
...
#define USER_VQ_RAMPUP_STEP                        (2U)                                             /*!< Vq voltage increment step in target count*/
#define USER_VQ_RAMPDOWN_STEP                      (5U)                                             /*!< Vq voltage decrement step in target count*/
...
#define USER_SPEED_RAMPUP_RPM_PER_S                (100U)
#define USER_SPEED_RAMPDOWN_RPM_PER_S              (100U)
...
#define USER_SVM_SWITCHING_SCHEME                   STANDARD_SVM_7_SEGMENT              /*!<1. STANDARD_SVM_7_SEGMENT          2. STANDARD_SVM_5_SEGMENT  */
...
#define USER_PLL_LPF                                (2)                                 /*!<0 - Filter Disabled, >0 - Filter enabled(Time const = PWM_PERIOD * 2^USER_PLL_LPF) */
#define USER_PLL_SPEED_LPF                          (2)                                 /*!<0 - Filter Disabled, >0 - Filter enabled(Time const = PWM_PERIOD * 2^USER_PLL_LPF) */


/* --------------------------------------------------- BRAKING CONFIG  -------------------------------------------------- */
#define USER_MOTOR_BRAKE_DUTY                       (60U)                                /*!< Brake duty percentage. Ex: 100 - Strong brake, 10 - Weak Brake  */
...
#define USER_UCPROBE_OSCILLOSCOPE                   DISABLED                              /*!< 1. ENABLED       2. DISABLED */
...
#define USER_VDC_LINK_V                             (36.0f) 					    /*!< Inverter DC link voltage in Volts  */
...
// CAUTION: The gain is defined here but also need to configure the same gain for 6EDL7141 in 6EDL_gateway.c
#define USER_CURRENT_AMPLIFIER_GAIN                 (8.0f) 					    /*!< R_FEEDBACK (of equivalent amplifier) kohm using 6EDL7141 CS gain of 4/8/12/16*/
#define USER_MAX_ADC_VDD_V                          (5.0f)						    /*!< VDD5, maximum voltage at ADC */

#if  (MOTOR0_PMSM_FOC_BOARD == EVAL_IMD700A_FOC_3SH)
#define USER_R_SHUNT_OHM                            (0.020f)					    /*!< Phase shunt resistor in ohm, eval board 0.010 */
...
#define USER_PI_SPEED_KP                            (18000)                          /*!<  SPEED PI CTRL - Proportional gain Kp, uint16_t. */
#define USER_PI_SPEED_KI                            (1)                             /*!<  SPEED PI CTRL - Integral gain Ki, uint16_t. */
#define USER_PI_SPEED_SCALE_KPKI                    (12)                            /*!<  SPEED PI CTRL - kp,ki scale, uint16_t. */

#define USER_PI_PLL_KP                              (100)                            /*!<  PLL PI CTRL - Proportional gain Kp, uint16_t. */
...
#define PLL_KP_ADJUSTMENT                           ENABLED
#ifdef  PLL_KP_ADJUSTMENT
/* Define parameters for on-line PLL adjustment */
// Define the speed points and the corresponding PLL Kp value
#define NUM_SEGMENTS 4                              // define the segment number
static const int32_t speedPoints[NUM_SEGMENTS + 1] = {1251, 3977, 8651, 16440, 32767};
static const int32_t pllKpPoints[NUM_SEGMENTS + 1] = {100, 50, 30, 20, 10};
#define SCALE_PLL_KP_ADJUSTMENT 12

#endif

#define USER_PI_POSITION_KP                              (30000)
#define USER_PI_POSITION_KI                              (1024)
#define USER_PI_POSITION_SCALE_KPKI                    (9)
#define USER_PI_POSITION_UK_LIMIT_MIN                       (0);
#define USER_PI_POSITION_UK_LIMIT_MAX                       (32767);
#define USER_PI_POSITION_IK_LIMIT_MIN                       (-(1<<(30 - USER_PI_POSITION_SCALE_KPKI)))       /* I[k] output limit LOW. */ 
#define USER_PI_POSITION_IK_LIMIT_MAX                       (1<<(30 - USER_PI_POSITION_SCALE_KPKI))          /* I[k] output limit HIGH. */
/*********************************************************************************************************************
 * Below Parameters usually not required any change
 ********************************************************************************************************************/
/*--------------------------------------------------- Speed PI Controller Parameters ---------------------------------------- */
#define USER_PI_SPEED_UK_LIMIT_MIN                       0 //(-32768)                                   /* (-32767), 16. U[k] output limit LOW. */
...
#define USER_PI_TORQUE_UK_LIMIT_MIN                      (-8000)                                        /* U[k] output limit LOW. Normally no need change, originally -32768*/
...
#define USER_PI_PLL_IK_LIMIT_MIN                        (-(int32_t)(1U << (30U-USER_PI_PLL_SCALE_KPKI)))  //(-(1<<(30))) /* I[k] output limit LOW. */
